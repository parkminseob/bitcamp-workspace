...
public class BoardDaoImpl implements com.eomcs.pms.dao.BoardDao{
  ...
  public int insert(Board board) throws Exception {
    ...
    try (SqlSession sqlSession = sqlSessionFactory.openSession()) {
      ...
      // SqlSession객체에게 insert Sql문을 실행하라고 명령한다.
      return sqlSession.insert("BoardMap.insert", board);
    }
  }
  ...
  public List<Board> findAll() throws Exception {
    // mybatis 객체 준비
    // => mybatis 설정파일을 읽어 들일 입력 스트림을 준비
    InputStream inputStream = Resources.getResourceAsStream("com/eomcs/pms/conf/mybatis-config.xml");

    // => 입력스트림에서 mybatis설정값을 읽어 SqlSessionFactory를 만든다.
    SqlSessionFactory sqlSessionFactory =
        new SqlSessionFactoryBuilder().build(inputStream);

    // => SqlSessionFactory에서 SqlSession객체를 얻는다.
