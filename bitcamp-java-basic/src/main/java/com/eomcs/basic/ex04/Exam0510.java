...
public class Exam0510 {
  public static void main(String[] args) {
    ...
    int p3[]; // c언어 방식도 사용가능. 단 개수를 지정해선 안됨 
    ...
    // 자바에서는 원시타입을 제외한 모든 변수가 레퍼런스이다!
    
    String str = "okok"; //str이 레퍼런스임! 
    
    // 레퍼런스 변수의 주소 값을 0으로 초기화하기.
    // p2 = 0; //이렇게 직접적으로 초기화 시킬 수 없다.
    p2 = null; // 주소 변수(레퍼런스)를 0으로 초기화시키려면 null을 사용하라.
    // 0이라고 직접 적으면 레퍼런스 변수가 아니고 int변수같아보여! 그래서 null로 셋팅하는거임.
    // System.out.println(p2[0]); // NullponiterException = > runtime exception
    ...
    int[] k5 = {11, 12, 13}; //new int[] {11, 12, 13} 과 같은 문장임!
    ...
    // = garbage collector
    // 단 바로 해제시키는 것이 아니라 다음 조건에 해당될때 해제시킨다.
    // 1) 메모리가 부족할 떄
    // 2) 시스템이 한창동안 한가할 때
    // => 결론! 언제 garbage를 메모리에서 해제시킬지 알 수 없다.
    
    System.gc(); // 가능하다면 빠른 시일 내에 가비지를 수집해 줄 것을 요구하는것!
    // 주의 ! 
