...
public class Exam0410 {
  static class SmartPhone{
    ...
    //    제품이 구동되는데 문제가 없도록 기본 값으로 설정되어야 함.
    // - 클래스 설계도에 따라 인스턴스를 생성한 후에
    // - 그래서 인스턴스를 사용하기 전에 적절한 값으로 초기화시키기 위해
    //   자동으로 호출되는 메서드를 도입하게됨.
    //   이것이 생성자다!!!!

    // 생성자 문법
    // - 클래스명(파라미터, ....) {}
    // - 생성자를 선언할 때는 리턴 타입을 지정하지 않음.
    ...
    // 예 ) public 클래스명() {}
    //
    // 이 클래스에 생성자를 정의하지 않으면 다음과 같이 기본 생성자가 추가됨
    // public smartPhone() {}
  ...
  public static void main(String[] args) {
    ...
    //안하면 문법오류임
    // => new 클래스명(생성자에 넘겨줄 값);
    // 예) new A(); <-- 생성자에 파라미터가 없기 때문에 값을 넘겨주지않음.
    ...
    // 다음과 같이 호출될 생성자를 지정하지 않으면 문법 오류임
    ...
    SmartPhone obj1 = new SmartPhone(/*이곳에 생성자값이 들어간*/); //파라미터가 없는 생성자.
    obj1.playMusic();
