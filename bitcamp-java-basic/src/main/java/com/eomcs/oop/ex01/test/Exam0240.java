...
public class Exam0240 {
  ...
  public static void main(String[] args) {
    ...
    // 메서드에서 생성한 Score객체를 레퍼런스 배열의 각 항목에 저장한다.
    arr[0] = createScore("홍길동", 100, 100, 100);
    arr[1] = createScore("임꺽정", 90, 90, 90);
    arr[2] = createScore("유관순", 80, 80, 80);
    
    // 매서드에 배열을 통째로 넘길 수 있다.
    // 정확하게 표현하면, arr변수에 저장된 레퍼런스 배열의 주소를 넘긴다.
    printScore(arr);
  }
  ...
  static void printScore(Score[] arr) {
    // main()에서 넘겨준 레퍼런스 배열의 주소를 arr변수를 받는다.
